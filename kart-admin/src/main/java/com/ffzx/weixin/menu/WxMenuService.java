...
public class WxMenuService {
    ...
    public static void createMenu() {
        ...
        menu.put("type", "view");
        menu.put("url", "http://kart.ffzxnet.com");
        menu.put("name", "首页");
        menuList.add(menu);

        menu = new HashMap<>();
        menu.put("type", "view");
        menu.put("url", "http://kart.ffzxnet.com/center.html");
        menu.put("name", "个人中心");
        menuList.add(menu);

        menu = new HashMap<>();
        menu.put("type", "view");
        menu.put("url", "http://kart.ffzxnet.com/siteChoose.html");
        menu.put("name", "在线预订");
        menuList.add(menu);
    ...
    public static void createMenu(List<com.ffzx.kart.model.WxMenu> menus) {
    ...
    protected static List<WxMenu> preProcess(List<com.ffzx.kart.model.WxMenu> menus) {
        Collections.sort(menus,new WxMenuComplexComparetor());
        List<WxMenu> list = new ArrayList<>();
        Map<String, com.ffzx.kart.model.WxMenu> menuMap = new HashMap<>();
        Map<String, WxMenu> wxMenuMap = new HashMap<>();
        for (com.ffzx.kart.model.WxMenu m : menus) {
            menuMap.put(m.getId(), m);
            if (StringUtils.isBlank(m.getpId())) {
                WxMenu wxM = convert(m);
                list.add(wxM);
                wxMenuMap.put(m.getId(), wxM);
            }
        }
        for (com.ffzx.kart.model.WxMenu m : menus) {
            menuMap.put(m.getId(), m);
            if (StringUtils.isNotBlank(m.getpId())) {
                WxMenu wxM = wxMenuMap.get(m.getpId());
    ...
    public static WxMenu convert(com.ffzx.kart.model.WxMenu menu) {
    ...
    static class WxMenuLevelComparetor implements Comparator<com.ffzx.kart.model.WxMenu> {

        @Override
        public int compare(com.ffzx.kart.model.WxMenu o1, com.ffzx.kart.model.WxMenu o2) {
            if (StringUtils.isBlank(o1.getpId())) {
                if (StringUtils.isNotBlank(o2.getpId())) {
                    return 1;
                }

            } else if (StringUtils.isNotBlank(o1.getpId())) {
                if (StringUtils.isBlank(o2.getpId())) {
                    return -1;
                }
            }
            return 0;
    ...
    static class WxMenuSortComparetor implements Comparator<com.ffzx.kart.model.WxMenu> {
    ...
    static class WxMenuComplexComparetor implements Comparator<com.ffzx.kart.model.WxMenu> {
        ...
        List<Comparator<com.ffzx.kart.model.WxMenu>> comparators = new ArrayList<>();
        ...
        public int compare(com.ffzx.kart.model.WxMenu o1, com.ffzx.kart.model.WxMenu o2) {
            ...
            for (Comparator<com.ffzx.kart.model.WxMenu> comp : comparators) {
