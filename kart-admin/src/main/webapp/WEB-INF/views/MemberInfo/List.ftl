...
<form id="find-page-orderby-form" action="${BasePath !}/MemberInfo/queryData.do" method="post" class="ff-form">
...
<script type="text/javascript" src="${BasePath !}/asset/js/kart/memberInfo/memberInfo_list.js?v=${ver !}"></script>
<style>.clm_action {
    ...
    $(document).ready(function () {
        ...
        ffzx.ui(['datatable'], function () {
            ...
            var dt_role_list = new initDataTable({
                div_id: 'data_list',
                url: rootPath + "/MemberInfo/queryData.do",
                columns: [
                    {data: "name", label: '用户名称', class: 'text-nowrap'},
                    {data: "idNumber", label: '身份证号码', class: 'text-nowrap'},
